pipeline {
    ...
    environment {
        ...
        EC2_HOST = "13.233.93.128"      // Replace with your EC2 instance IP
        APP_DIR = "/home/ubuntu/first-project/app" // Path to app directory on EC2
        SSH_KEY = "/var/lib/jenkins/mhv.pem"    // Path to your EC2 private key
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkins.pem -r app/main.py app/templates ubuntu@13.233.93.128:/home/ubuntu/first-project/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkins.pem ubuntu@13.233.93.128 <<EOF
