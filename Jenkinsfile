pipeline {
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/server.pem -r app/main.py app/templates ubuntu@13.233.93.128:/home/ubuntu/first-project/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/server.pem ubuntu@13.233.93.128 <<EOF
